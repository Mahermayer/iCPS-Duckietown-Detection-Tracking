...
def linear_assignment(cost_matrix, thresh):
    ...
    return np.asarray(matches), np.asarray(sorted(unmatched_a)), np.asarray(sorted(unmatched_b))
...
def ious(atlbrs, btlbrs):
    ...
    """
    # cython_bbox expects float64 buffers
    ious = np.zeros((len(atlbrs), len(btlbrs)), dtype=float)
    if ious.size == 0:
        return ious

    a = np.ascontiguousarray(atlbrs, dtype=float)
    b = np.ascontiguousarray(btlbrs, dtype=float)
    if bbox_ious is not None:
        return bbox_ious(a, b)

    area_a = np.maximum(0.0, a[:, 2] - a[:, 0]) * np.maximum(0.0, a[:, 3] - a[:, 1])
    area_b = np.maximum(0.0, b[:, 2] - b[:, 0]) * np.maximum(0.0, b[:, 3] - b[:, 1])
    for i, box_a in enumerate(a):
        xx1 = np.maximum(box_a[0], b[:, 0])
        yy1 = np.maximum(box_a[1], b[:, 1])
        xx2 = np.minimum(box_a[2], b[:, 2])
        yy2 = np.minimum(box_a[3], b[:, 3])
